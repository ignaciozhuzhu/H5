...
.controller('AppCtrl', ['$scope', '$ionicModal', '$timeout', '$ionicScrollDelegate', function ($scope, $ionicModal, $timeout, $ionicScrollDelegate) {
...
//线路列表控制器,从搜索进来
.controller('LinelistsCtrl', ['$scope', '$http', 'filtbydaysev', function ($scope, $http, filtbydaysev) {
    followfunc();
    ...
    $scope.example1data0 = [];
    $scope.example1model0 = [];
    var funcallback0 = function (_responseche) {
        for (var i = 0; i < _responseche.length; i++) {
            $scope.example1data0[i] = { id: _responseche[i].name, label: _responseche[i].name };
        }
    }
    filtbydaysev.getagencies($http, $scope, funcallback0, my2data);
    //旅行社筛选框ed----------------------------------------------------------------------------------------------------------------
...
.controller('LinelistsCtrl2', ['$scope', '$http', 'filtbydaysev', function ($scope, $http, filtbydaysev) {
    followfunc();
...
//主页控制器
.controller('indexCtrl', ['$scope', '$http', '$ionicScrollDelegate', function ($scope, $http, $ionicScrollDelegate) {
    followfunc();
    tkdfunc(
    ...
    $scope.$on("$ionicView.loaded", function () {
        ...
        //空搜关键词
        var nghttp2 = "../../ajax/areaHandler.ashx?fn=getarea3list";
        $http.get(nghttp2).success(function (response) {
...
//线路详情控制器
.controller('lineDetailCtrl', ['$scope', '$http', '$sce', '$ionicScrollDelegate', function ($scope, $http, $sce, $ionicScrollDelegate) {
    followfunc();
    var url = location.href;
    ...
    $scope.historygoback = function () {
        ...
        else if (ent2detail == "domestic" || ent2detail == "outbound" || ent2detail == "inbound" || ent2detail == "freetrip" || ent2detail == "sales") {
            window.location.href = "#/app/linelists/" + ent2detail;
            location.reload();
        }
    ...
    $http.get(nghttp).success(function (response) {
        ...
        for (var mi = 0; mi < smalllen; mi++) {
            days4.push(response.line.groups[mi]);
        }
        $scope.group1 = days4;
        //大于4的话就取出剩余的
        if (response.line.groups.length > 4) {
            for (var ni = 4; ni < response.line.groups.length; ni++) {
                daysmore.push(response.line.groups[ni]);
            }
        }
    ...
    $scope.collect = function () {
        ...
        if (!$scope.ifcollect) {
            ...
            var nghttp2 = "../../ajax/apihandler.ashx?fn=cancelcollect&lineID=" + lineID + "&userMID=" + userMID + "";
            var mylayeruiwait2 = layer.load(1, {
                shade: [0.5, '#ababab'] //0.1透明度的白色背景
            });
            $http.get(nghttp2).success(function (response) {
                layer.close(mylayeruiwait2);
                if (response == "操作成功!") {
                    collectstatus(false);
                }
            })
    ...
    $scope.$on("$ionicView.loaded", function () {
        ...
        $http.get(nghttp).success(function (response) {
            ...
            if (!response.line.images.length) {
                //layermyui('轮播图未设置');
                $('.linedetail #full-width-slider').append('<div class="rsContent"><img class="rsImg" src="img/线路详情默认图片.png" style="height:200px !important;width:100%" /></div>');
                return;
            }
            else {
...
//日期选择控制器
.controller('indexdateCtrl', ['$scope', '$http', function ($scope, $http) {
    nofollowfunc();
    $scope.$on("$ionicView.loaded", function () {
        var url = location.href;
        var lineid = url.substring(url.lastIndexOf('/') + 1, url.length);
...
//选择资源控制器2
.controller('pickresourceCtrl2', ['$scope', '$http', function ($scope, $http) {
    nofollowfunc();
    ...
    $http.get(nghttp).success(function (response) {
        ...
        $("#sp1").click(function () {
            ...
            subamount();
            $scope.difnum = roomdiffp1;
            if (roomdiffp1 == "0")
                $(".dfcbox").css("display", "none")
            else
                $(".dfcbox").css("display", "block")
        });
        $("#sp2").click(function () {
        ...
        //初始状态
        $(".black_overlay").css("height", $(".mytopdiv").height());
        if (cnum == "0")
            $(".rtbox").css("display", "none")
        else
            $(".rtbox").css("display", "block")
        $(".dfcbox").css("display", "none")
       
    })
    $scope.getcoup = function () {
        if (parseInt($("#sp1")[0].children[0].children[1].value)) {
            layermyui('选择优惠券与选择单房差不能交叉操作');
            return;
        }
    ...
    $scope.nonecoup = function () {
        ...
        $('.pickresource .amount').empty().append(amountall);
        setCookie('coupamount', "", 1);
        $(".pickresource #sp1")[0].children[0].children[2].disabled = false;
    }

    //$scope.costdetail = function (a) {
    //    $(".black_overlay").css('display', 'block');
    //    $("#costdetail").css('display', 'block');
    //}

    $scope.costdetailnone = function (event) {
        if (event.srcElement.innerHTML == "费用明细") {
            $(".black_overlay").css('display', 'block');
            $("#costdetail").css('display', 'block');
        }
        else {
...
//填写订单控制器
.controller('fillorderCtrl', ['$scope', '$http', function ($scope, $http) {
    nofollowfunc();
    ...
    if (getCookie('phonenum'))
        $scope.Connect.mobile = getCookie('phonenum');
    ...
    $scope.createorder = function () {
        ...
        var gueststring = "";
        for (var hi = 0; hi < pnum; hi++) {
            gueststring += "{\"category\":\"" + guestsarr[hi].category + "\",\"name\":\"" + guestsarr[hi].name + "\"},";
        }
        ...
        //先检测是否是会员手机号,是就跳登录或下单.否就直接注册并下单.
        var nghttp = "../../ajax/userHandler.ashx?fn=getvalidateemailortel&phone=" + ConnectMobile + "";
        $http.get(nghttp).success(function (text) {
            // debugger
            find404admin(text);
            layer.close(mylayeruiwait);
            var jsonObj = new X2JS().xml_str2json(text);
            if (jsonObj.validateEmailOrPhoneRespDto.existFlag == "false") {
                //自动注册并下单
                // layermyui('正在注册并下单');
                quickregist();
            }
            ...
            function quickregist() {
                ...
                var xml = "<webMemberDto><phone>" + ConnectMobile + "</phone><registChannel>Website</registChannel></webMemberDto>";
                //先检测是否是会员手机号,是就跳登录或下单.否就直接注册并下单.
                var nghttp = "../../ajax/userHandler.ashx?fn=quickregistorder&xml=" + xml + "";
                $http.get(nghttp).success(function (response) {
                    //debugger
                    find404admin(response);
                    layer.close(mylayeruiwait);
                    var jsonObj = new X2JS().xml_str2json(response);
                    if (jsonObj.webMemberRegisterReturnDto.mcMemberCode) {
                        ...
                        setCookie('phonenum', ConnectMobile, 1);
                        //注册完了直接下单
                        createorder();
                    }
                    else {
                        layermyui('下单注册失败,请联系管理员!');
                    }
                })
            }
...
//支付方式控制器
.controller('paywayCtrl', ['$scope', '$http', 'getbindquerysev', function ($scope, $http, getbindquerysev) {
    nofollowfunc();
    //blockmyui('正在加载,请稍后...');
    //清除登录用户cookie
        ...
        $scope.pay = function () {
            ...
            if (mcMemberCode != "" && mcMemberCode != undefined && mcMemberCode != null) {
                ...
                if (accountName == "JJE_APP_UNION_PAY") {
                    ...
                    //测试mock数据
                    amount = 0.01;
                    var nghttpt = "../../ajax/apihandler.ashx?fn=pbppayorder&orderNo=" + orderNo + "&payAmount=" + amount + "&accountName=" + accountName + "";
                    //loading层
                    var mylayeruiwait3 = layer.load(1, {
                        shade: [0.5, '#ababab'] //0.1透明度的白色背景
                    });
                    $http.get(nghttpt).success(function (response) {
                        layer.close(mylayeruiwait3);
                        var jsonObj = new X2JS().xml_str2json(response);
                    ...
                    //测试mock数据
                    amount = 1;
                    var nghttpm = "../../ajax/apihandler.ashx?fn=pbppayorder&orderNo=" + orderNo + "&payAmount=" + amount + "&accountName=" + accountName + "";
                    //loading层
                    var mylayeruiwait2 = layer.load(1, {
                        shade: [0.5, '#ababab'] //0.1透明度的白色背景
                    });
                    $http.get(nghttpm).success(function (response) {
                        layer.close(mylayeruiwait2);
...
//取消订单控制器
.controller('cancelorderCtrl', ['$scope', '$http', function ($scope, $http) {
    nofollowfunc();
